...
const account1 = {
  owner: 'Harsh Ranjan',
  movements: [200, 455.23, -206.5, 5000, -542.21, -130.9, 79.97, 1300],
  interestRate: 4.2, // %
  pin: 1111,
  movementsDates: [
...
const account2 = {
  owner: 'Raghav Raj',
  movements: [2000, 3400, -150, -790, -3210, -1000, 4500, -30],
  interestRate: 3.5,
  pin: 2222,
  movementsDates: [
...
const formatMovementDate = function (date) {
  ...
  // return new Intl.DateTimeFormat(locale).format(date); //  Using Internationalization API

  // //  Getting locale from user browser
...
const startLogOutTimer = function () {
  const tick = function () {
    ...
    //  Decreasee 1 sec in each call
    time--;
  };

  //  Set timer to 5 min (300 sec)
  let time = 300;
...
//  Event Handler_________________________👇
...
btnLogin.addEventListener('click', function (e) {
  ...
  //type conversion to number can be done by using a + sign also
  ...
  } else alert('!! incorrect username or PIN !!');
});

btnTransfer.addEventListener('click', function (e) {
  e.preventDefault();
  const amount = +inputTransferAmount.value; //  '+' for type conversion to number
...
btnLoan.addEventListener('click', function (e) {
  ...
  if (amount > 0 && currentAccount.movements.some(mov => mov >= amount * 0.1)) {
    setTimeout(function () {
      ...
      //  Update UI after 3 sec
      timer = startLogOutTimer();
    }, 3000);
  } else alert('!! Amount beyond limit !!'); //  (valid amount : max deposite is at least 10% of loan amount)
